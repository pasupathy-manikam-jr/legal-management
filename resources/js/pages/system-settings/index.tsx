        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-1 flex-col gap-4 px-3 pt-4 pb-12 lg:px-8">
                <div>
                    ...
                    <p className="text-muted-foreground text-xs">Firm-wide configuration. Your own account settings live under Settings.</p>
                </div>
                            ...
                            <div className="bg-card flex flex-col gap-2 rounded-lg border p-3">
                                {SECTIONS.map((section) => (
                                    <a
                                        ...
                                        className={cn(
                                            'hover:bg-accent flex h-9 w-full items-center gap-2 rounded-md px-4 text-sm transition-colors',
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            >
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                    <TextField
                                        label="Firm Name"
                                        value={brand.data.firm_name}
                                        onChange={(v) => brand.setData('firm_name', v)}
                                        error={brand.errors.firm_name}
                                    />
                                    <TextField
                                        label="Title Text"
                                        value={brand.data.title_text}
                                        onChange={(v) => brand.setData('title_text', v)}
                                        error={brand.errors.title_text}
                                    />
                            ...
                            >
                                <div className="bg-muted/30 mb-6 flex flex-col items-center justify-between gap-3 rounded-md border p-4 md:flex-row">
                                    ...
                                    <div className="text-muted-foreground text-xs">Saved value renders as {money(123456)}</div>
                                </div>
                                    ...
                                    <Field label="Symbol Position">
                                        <div className="flex flex-wrap gap-2">
                                            {(['before', 'after'] as const).map((position) => (
                                                ...
                                                >
                                                    {position === 'before'
                                                        ? `${currency.data.currency_symbol}100`
                                                        : `100${currency.data.currency_symbol}`}
                                                </Button>
                                            ))}
                                        </div>
                                    </Field>
                            ...
                            >
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                    <TextField
                                        label="Mail Driver"
                                        value={email.data.mail_driver}
                                        onChange={(v) => email.setData('mail_driver', v)}
                                        error={email.errors.mail_driver}
                                    />
                                    <TextField
                                        label="SMTP Host"
                                        value={email.data.mail_host}
                                        onChange={(v) => email.setData('mail_host', v)}
                                        error={email.errors.mail_host}
                                    />
                                    <TextField
                                        label="SMTP Port"
                                        type="number"
                                        value={email.data.mail_port}
                                        onChange={(v) => email.setData('mail_port', v)}
                                        error={email.errors.mail_port}
                                    />
                                    <TextField
                                        label="SMTP Username"
                                        value={email.data.mail_username}
                                        onChange={(v) => email.setData('mail_username', v)}
                                    />
                                    <TextField
                                        label="SMTP Password"
                                        type="password"
                                    ...
                                    <TextField
                                        label="From Name"
                                        value={email.data.mail_from_name}
                                        onChange={(v) => email.setData('mail_from_name', v)}
                                        error={email.errors.mail_from_name}
                                    />
                                </div>
                                ...
                                <div className="mt-6 rounded-lg border p-4">
                                    ...
                                    <p className="text-muted-foreground mt-2 text-xs">Uses the saved settings above, so save before testing.</p>
                                </div>
                            </Section>
                            ...
                            <Section id="notifications" title="Email Notifications" description="Choose which notifications the firm sends">
                                <div className="grid grid-cols-1 gap-4 min-[1200px]:grid-cols-2 min-[1500px]:grid-cols-3">
                                    {templates.length === 0 && <p className="text-muted-foreground text-sm">No templates configured.</p>}
                                    {templates.map((template) => (
                                        <div key={template.id} className="flex items-center justify-between gap-3 rounded-lg border p-3">
                                            ...
                                            <Switch
                                                ...
                                                onChange={() =>
                                                    router.patch(`/system-settings/templates/${template.id}`, {}, { preserveScroll: true })
                                                }
                                            />
                                        </div>
                                    ))}
                                </div>
                                <p className="text-muted-foreground mt-3 text-xs">Edit the wording of each template under Notification Templates.</p>
                            </Section>
                            ...
                            >
                                ...
                                <div className="mt-6 flex flex-wrap items-center gap-3 rounded-lg border p-4">
                                    <div className="min-w-[200px] flex-1">
                                        ...
                                        <p className="text-muted-foreground mt-1 text-xs">Posts a test message to the saved webhook.</p>
                                    </div>
                                    <Button
                                        type="button"
                                        variant="outline"
                                        onClick={() => router.post('/system-settings/test-slack', {}, { preserveScroll: true })}
                                    >
                            ...
                            >
                                ...
                                <div className="mt-6 rounded-lg border p-4">
                                    ...
                                    <p className="text-muted-foreground mt-2 text-xs">
                                        Uses the saved credentials above, so save before testing. Twilio charges for the message.
                                    </p>
                                </div>
                            </Section>
                            ...
                            >
                                ...
                                <div className="mt-6 flex flex-wrap items-center gap-3 rounded-lg border p-4">
                                    <div className="min-w-[200px] flex-1">
                                        ...
                                        <p className="text-muted-foreground mt-1 text-xs">Asks Stripe whether the saved secret key is valid.</p>
                                    </div>
                                    <Button
                                        type="button"
                                        variant="outline"
                                        onClick={() => router.post('/system-settings/test-stripe', {}, { preserveScroll: true })}
                                    >
                            ...
                            >
                                ...
                                <div className="mt-4">
                                    ...
                                    <p className="text-muted-foreground mt-1.5 text-xs">
                                        A note of which calendar the feed was added to. Nothing is sent to Google.
                                    </p>
                                </div>
                                ...
                                <div className="mt-6 rounded-lg border p-4">
                                    ...
                                    <p className="text-muted-foreground mt-1 text-xs">
                                        In Google Calendar choose <span className="font-medium">Other calendars → From URL</span> and paste this
                                        address. Anyone holding the link can read the firm&apos;s hearings, so treat it as a password.
                                    </p>
                                    {calendarFeedUrl ? (
                                        ...
                                        <p className="text-muted-foreground mt-3 text-xs">Turn the feed on and save to generate a link.</p>
                                    )}
                                </div>
                            </Section>
                        </div>
