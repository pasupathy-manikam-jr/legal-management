...
export default function Templates({
    ...
    const apply = (patch: Record<string, string | number>) =>
        router.get('/settings/templates', { ...filters, channel, per_page: perPage, direction: sort.direction, ...patch }, { preserveState: true, replace: true });
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-1 flex-col gap-4 px-3 pt-4 pb-12 lg:px-8">
                <div>
                    ...
                    <p className="text-xs text-muted-foreground">Manage email and system notification templates for automated messages.</p>
                </div>
                ...
                <div className="rounded-lg border bg-card shadow-sm">
                    <div className="flex items-center justify-between gap-2 p-3">
                        ...
                        <div className="mr-2 rounded-md border p-0.5">
                            <div role="tablist" className="grid h-10 w-fit items-center gap-1 rounded-md bg-muted p-1 text-muted-foreground" style={{ gridTemplateColumns: `repeat(${channels.length}, minmax(0, 1fr))` }}>
                                {channels.map((value) => (
                                    <button
                                        key={value}
                                        type="button"
                                        ...
                                        onClick={() => {
                                            setSearch('');
                                            router.get('/settings/templates', { channel: value, per_page: perPage }, { preserveState: true, replace: true });
                                        }}
                                        className={cn(
                        ...
                        <table className="w-full caption-bottom text-sm">
                            ...
                            <tbody className="divide-y">
                                ...
                                {templates.data.map((template, index) => (
                                    <tr key={template.id} className="transition-colors hover:bg-muted/40">
                                        <td className="px-4 py-2.5 font-medium">{(templates.from ?? 1) + index}</td>
                                        <td className="px-4 py-2.5">
                                            <span className={cn('text-sm font-medium', !template.active && 'text-muted-foreground line-through')}>{template.name}</span>
                                        </td>
                                        <td className="px-4 py-2.5">
                                            <div className="flex items-center justify-end">
                                                <Button variant="ghost" size="icon" className="size-8 text-muted-foreground" title="View" onClick={() => open(template)}>
                ...
                >
                    ...
                    <TextareaField label="Message" value={form.data.body} onChange={(v) => form.setData('body', v)} error={form.errors.body} className="sm:col-span-2" />
                    <p className="text-xs text-muted-foreground sm:col-span-2">
                        Variables:{' '}
                        {variables.map((v) => (
                            <code key={v} className="mr-1 rounded bg-muted px-1 py-0.5 font-mono text-[11px]">
                                {v}
                            </code>
                        ))}
                    </p>
