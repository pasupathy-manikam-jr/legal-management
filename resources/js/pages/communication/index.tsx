            ...
            <div className="flex flex-1 flex-col gap-4 px-3 pt-4 pb-12 lg:px-8">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground">Manage and track conversations and communication history.</p>
                    </div>
                ...
                <div className="flex h-[calc(100vh-220px)] overflow-hidden rounded-xl border bg-muted/30">
                    <aside
                        className={cn(
                            'flex w-full flex-col border-r bg-card min-[1000px]:w-80',
                            selected && 'hidden min-[1000px]:flex',
                        )}
                    >
                        ...
                        <div className="flex-1 overflow-y-auto">
                            {conversations.length === 0 && <p className="p-6 text-center text-sm text-muted-foreground">No conversations yet.</p>}
                            {conversations.map((c) => (
                                <button
                                    key={c.id}
                                    onClick={() => openThread(c.id)}
                                    className={cn(
                                        'flex w-full cursor-pointer items-center p-4 text-left transition-colors hover:bg-accent/50',
                                ...
                                >
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <div className="mt-1 flex items-center justify-between gap-2">
                                            <p className="flex-1 truncate text-sm text-muted-foreground">{c.preview ?? 'No messages yet.'}</p>
                                            {c.unread > 0 && (
                    ...
                    <section className="flex flex-1 flex-col">
                        ...
                        {selected && (
                            <>
                                <header className="flex h-[65px] items-center gap-3 border-b bg-card px-4">
                                    ...
                                    <Button
                                        ...
                                        onClick={() => confirmAction({ title: 'Delete this conversation and its messages?' }).then((ok) => ok && router.delete(`/messages/${selected.id}`))}
                                    >
                                ...
                                <div className="flex-1 space-y-3 overflow-y-auto p-4">
                                    {selected.messages.length === 0 && (
                                        <p className="py-10 text-center text-sm text-muted-foreground">Nothing sent yet.</p>
                                    )}
                                            ...
                                            <div key={m.id} className={cn('flex', outbound ? 'justify-end' : 'justify-start')}>
                                                <div
                                                    className={cn(
                                                        ...
                                                        outbound ? 'rounded-br-sm bg-primary text-primary-foreground' : 'rounded-bl-sm bg-card',
                                                    )}
                                                >
                                ...
                                <form onSubmit={submit} className="flex items-center gap-2 border-t bg-card p-3">
                                    <Input
